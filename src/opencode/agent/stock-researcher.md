...
- **Ticker may not be a stock (ETF / fund)**: A ticker can resolve to a European-listed UCITS ETF or bond fund (e.g. `SXRH` = iShares USD TIPS 0-5 UCITS ETF). Finviz, Stocktwits, and the StockAnalysis *stock* page all 404, and `finance.yahoo.com/quote/<TICKER>.DE/` (foreign listing) often fails too -- this signals a non-equity instrument, not a dead ticker. Route to **justETF** (`justetf.com/en/etf-profile.html?isin=<ISIN>` or `?query=<TICKER>`) as the primary source. Equity fields (P/E, RSI, short float/squeeze, analyst targets, Stocktwits, insider/institutional %) are genuinely N/A -- mark them so and adapt the framework to ETF/fixed-income metrics: TER, AUM, distribution yield, effective duration, credit quality, index tracked, and FX exposure (an unhedged USD fund held in EUR carries large USD/EUR swing -- often the dominant driver of headline return). Use the Acc/Dist sibling share class and the US-listed equivalent (e.g. `STIP` for short TIPS) as data proxies for price action, beta, and yield.
- **Ticker may be a foreign-listed stock (Oslo / Euronext / LSE / etc.)**: A bare ticker like `NORSE` can be the *home-exchange* symbol of a non-US company (Norse Atlantic ASA on Oslo Børs), not a US equity. Yahoo `quote/<TICKER>/` returns a symbol-lookup / "Oops" page that lists the real listings (e.g. `NORSE.OL` primary, OTC `NRSAF`), and Finviz / Stocktwits / `stockanalysis.com/stocks/<TICKER>/` all 404. This is a routing problem, not a dead ticker. Resolve via the Yahoo lookup, then pull the primary listing from **StockAnalysis foreign-exchange URLs**: `stockanalysis.com/quote/<exchange>/<TICKER>/` (e.g. `osl/NORSE`) plus `/statistics/` and `/financials/?p=quarterly`. Use the US OTC/ADR symbol (e.g. `NRSAF`) for the Stocktwits line. Report in the home currency (NOK) with approximate USD conversions, and reconcile market cap via `shares × home-currency price` (Stocktwits OTC market cap is often badly stale on illiquid lines). Short interest is genuinely N/A for non-US listings -- mark it so.
- **StockAnalysis `/statistics/` is a full Finviz replacement** when Finviz 404s (foreign primaries, OTC pink sheets): `stockanalysis.com/quote/<exchange>/<TICKER>/statistics/` supplies RSI, 50/200-day MA (as absolute price *levels*, not %), beta, Altman Z-Score, Piotroski F-Score, current/quick ratio, full balance sheet (cash, debt, equity, working capital), ownership %, and dilution (shares change YoY/QoQ) -- everything except short interest.
- **TIPS / inflation-linked bond yields are overstated**: A TIPS ETF's trailing distribution yield is inflated by inflation-linked principal accrual and is *not* a clean real-income figure. Quote the index/USD yield alongside it and flag that distributions fluctuate with CPI -- never present the TTM distribution yield as the forward income rate.
- **Stale analyst price targets on collapsed stocks**: StockAnalysis/Yahoo can surface an unmaintained consensus target implying absurd upside (e.g. a NOK 13.61 "Buy" / +2,000% target on a NOK 0.63 distressed name in a strategic review). When the implied upside is multi-hundred/thousand-percent against a price that has already collapsed, treat the target as a stale data artifact -- flag it as such rather than reporting the headline upside as a live call.
- **Management adjusted metrics vs reported IFRS/GAAP**: Press-release narratives (especially airlines and capital-intensive names) tout favorable adjusted figures -- EBITDAR (ex-aircraft rent), "scheduled revenue" growth, adjusted EBITDA -- that conflict with the data provider's reported numbers (Norse claimed "+66% revenue, return to positive EBITDA" while S&P showed +28% revenue and -$12.5M EBITDA for the same quarter). Cross-check management claims against StockAnalysis/S&P reported figures and flag the divergence; never repeat the rosy press-release metric as the GAAP/IFRS result.
