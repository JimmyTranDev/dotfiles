...
1. Run `detect-stack.sh` to identify the project type, package manager, test runner, and linter.

2. Run all checks in parallel (use parallel tool calls — these have no dependencies between them):
   - **Tests**: `run-tests.sh`
   - **Type checking**: `npx tsc --noEmit` / `pnpm tsc --noEmit` (for TypeScript projects, using the package manager from `detect-stack.sh`)
   - **Linting**: `lint-check.sh`
...
4. Fix issues iteratively:
   ...
   - **Lint errors**: Apply auto-fix first (`lint-check.sh --fix`), then manually fix remaining issues
