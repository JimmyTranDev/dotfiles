...
- **Todoist priority filter** — when a `p1`, `p2`, `p3`, or `p4` token appears alongside a Todoist section URL, filter fetched tasks to only those matching that priority. Use the inverted API mapping from the todoist-cli skill (p1=4, p2=3, p3=2, p4=1 in JSON). If no priority token is present, fetch all tasks in the section.
