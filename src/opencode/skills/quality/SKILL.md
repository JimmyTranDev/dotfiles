...
|----------|-----------------|-------------------|
| Naming clarity | Vague variable/function names, inconsistent conventions, misleading identifiers | **pragmatic-programmer** (Naming as Documentation) |
| Function design | Functions doing too much, unclear responsibilities, deep nesting, high cyclomatic complexity | **simplifier** (Code Smell Detection, Complexity Reduction) |
| Duplication | Repeated patterns, copy-pasted logic, similar implementations that should be unified | **deduplicator** (extraction rules and process) |
| Over-engineering | Premature abstractions, unnecessary indirection, wrappers that add no value | **consolidator** (inlining, layer collapsing) |
| Type safety | Loose types, `any` usage, unsafe casts, missing discriminated unions, overly broad generics | **total-typescript** |
| Dead code | Unused exports, unreachable branches, deprecated patterns, vestigial parameters | **simplifier** (Structural Smells) |
| Module structure | Tight coupling, poor cohesion, circular dependencies, barrel file bloat, unclear dependency direction | **consolidator** (file/layer decisions) |
| Architecture | Mixed abstraction levels, leaky abstractions, god objects, single responsibility violations | **pragmatic-programmer** (Orthogonality, Decoupling) |
...
- Detailed refactoring patterns and code smell catalogs — load the **simplifier** skill
- DRY extraction process and rules — load the **deduplicator** skill
- Inlining over-separated code — load the **consolidator** skill
- Design principles (DRY, orthogonality, reversibility) — load the **pragmatic-programmer** skill
- Logic correctness (contradictions, invalid assumptions) — load the **logic-checker** skill
- TypeScript-specific patterns (generics, branded types) — load the **total-typescript** skill
- ESLint configuration — load the **eslint-config** skill
- Shell script conventions — load the **shell-scripting** skill
