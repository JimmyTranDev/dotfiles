...
- **`yolo`** — a standalone, case-insensitive `yolo` token switches to the
  **autonomous** flow: no go/no-go gates; pause only for a genuinely blocking
  ambiguity (asked with the `question` tool). Strip it before reading the task.
  Absent → the **gated** flow (confirm after the spec and after the plan).
...
Run the **identical** core flow from `/implement`, honoring `yolo` (gated confirm
gates vs. autonomous clarify-only):

1. **Spec** — load `spec-driven-development`; surface assumptions, then a concise
   objective / testable success criteria / boundaries. **Gated:** confirm before
   planning.
2. **Plan** — load `planning-and-task-breakdown`; ordered, dependency-aware tasks,
   each S–M with acceptance + a verify step. **Gated:** confirm before building.
...
- **Honor the gates.** Gated by default (confirm after spec and after plan);
  `yolo` removes the gates but still clarifies genuinely blocking ambiguities.
...
| "Skip the spec/plan gate, it's obvious." | Gated by default — the cheapest place to catch a wrong assumption. `yolo` is the explicit opt-out. |
...
- Skipping the spec/plan confirm gates without the `yolo` modifier.
- Implementing features not in the spec, or fixing unrelated issues mid-build.

## Verification

- [ ] The change was specced, planned, built, verified, and reviewed (gates
      honored, or `yolo` clarify-only).
