...
- **Spec cleanup**: If `$ARGUMENTS` references files in `spec/` (paths starting with `spec/` or containing `.md` files inside `spec/`), delete each consumed spec file after all its tasks are successfully implemented and committed. If the `spec/` directory is empty after deletion, remove it too. Note in the final summary: "Removed consumed spec: spec/xyz.md"
