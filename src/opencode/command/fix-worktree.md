...
one optional modifier below. If no PR is identified, target the PR for the branch
...
3. **Choose the permission posture, then launch headless opencode** — pick, in
   ...
   code fixes + commit only); or full auto-approval (only with the user's explicit
   authorization). Do **not** reflexively reach for
   ...
   **targeting the worktree with `--dir`** so it never touches your main clone,
   ...
   ```
   Scope the delegated prompt to triage + code fixes + commit and hold the side
   effects for Phase 4 (the gated verify-then-push step).
