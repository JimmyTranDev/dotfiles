...
- **follower**: Always use first to study existing codebase conventions (naming, imports, file structure, patterns) so all new code matches the established style
- **structure**: Use when the task describes adding new modules, reorganizing files, or restructuring project layout
...
- **sounder**: Use when the task involves business logic, state machines, or complex conditional flows to verify logical soundness and catch impossible states
- **reuser**: Use when the task describes extracting shared utilities or reducing duplication across the codebase
- **pragmatic**: Use when the task describes refactoring or simplification work — apply DRY, KISS, YAGNI principles
...
- **re-export-destroyer**: Use when the task describes cleaning up barrel files, fixing circular dependencies, or optimizing imports
- **prompter**: Use when the task describes writing or updating AI system prompts or agent instructions

Workflow:
1. Analyze the prompt to categorize the type of work (feature, fix, refactor, test, security, performance, etc.)
2. Run the **follower** agent to learn codebase conventions
