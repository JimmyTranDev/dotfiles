...
- `~/.claude/` is a **symlink** to `src/claude/` (created by `etc/scripts/src/install/sync_links.sh`), so the Claude Code config is tracked directly in this repo. Inside it, `commands/` and `skills/` are symlinks into `src/opencode/command/` and `src/opencode/skills/` — OpenCode remains the single source of truth for those, so edit the files under `src/opencode/`. Runtime state (sessions, caches) is gitignored, and `settings.json` is symlinked to `secrets/claude/settings.json`.
