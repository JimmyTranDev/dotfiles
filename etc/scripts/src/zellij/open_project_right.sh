...
main() {
	...
	# falling back to an empty shell when nothing is recorded yet — same as
	# "Alt ]".
	local tool
	tool="$(last_pane_tool)" || tool="empty"
	[[ "$tool" != "empty" ]] && { require_tool "$tool" || exit 1; }

	# Take the project dir from the pane to the RIGHT (focus right, read, focus
	# back left), then open the stacked tool pane back here. Fall back to the
	# current pane, the last project, then the fzf picker when the right pane's
	# cwd can't be resolved (e.g. outside a tracked dir).
	local target_dir
